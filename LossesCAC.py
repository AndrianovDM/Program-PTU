...
def CR_inc(inlet_angle, outlet_angle, pitch, width, a_outlet):
    ...
    exit_angle_val = math.degrees(math.asin(a_outlet / pitch))
    return interp1d(exit_angle_range, y1, fill_value='extrapolate')(exit_angle_val).tolist()

# figure 2.22
def PW_inc_positive(pitch, width, a_outlet):
...
# figure 2.25
def PW_inc_negative(pitch, width, a_outlet):
...
# figure 2.21
def i_val_inlet_lower_90(blade_inlet_angle, pitch, a_outlet):
    ...
    y1 = [func(blade_inlet_angle) for func in f1]
    return interp1d(exit_angle_range, y1, fill_value='extrapolate')(math.degrees(math.asin(a_outlet / pitch))).tolist()

# figure 2.24
def stall_inc_negative(blade_inlet_angle, pitch, a_outlet):
...
# figure 2.26
def stall_inc_positive(blade_inlet_angle, pitch, a_outlet):
    ...
    i_stall_plus = interp1d(exit_angle_plus_range, y1, fill_value='extrapolate')(
        math.degrees(math.asin(a_outlet / pitch))).tolist()
...
# figure 2.20
def Npi(inlet_angle, outlet_angle, blade_inlet_angle, design_inc, pitch, width, a_outlet):
    plus_inc_i_stall_pitch_width = PW_inc_positive(pitch, width, a_outlet)
    plus_inc__i_stall_cr = CR_inc(inlet_angle, outlet_angle, pitch, width, a_outlet)
    minus_inc_i_stall_pitch_width = PW_inc_negative(pitch, width, a_outlet)
    if inlet_angle <= 91.0:
        plus_i_stall_basic = i_val_inlet_lower_90(blade_inlet_angle, pitch, a_outlet)
        plus_i_stall = plus_i_stall_basic + plus_inc_i_stall_pitch_width + plus_inc__i_stall_cr

        minus_i_stall_basic = stall_inc_negative(blade_inlet_angle, pitch, a_outlet)
        minus_i_stall = minus_i_stall_basic + minus_inc_i_stall_pitch_width
    else:
        plus_i_stall_basic, minus_i_stall_basic = stall_inc_positive(blade_inlet_angle, pitch, a_outlet)
        plus_i_stall = plus_i_stall_basic + (
                    1 - (inlet_angle - 90) / (90 - math.degrees(math.asin(a_outlet / pitch)))) * (
                               plus_inc_i_stall_pitch_width + plus_inc__i_stall_cr)
        minus_i_stall = minus_i_stall_basic + (1 - (inlet_angle - 90) / (
                90 - math.degrees(math.asin(a_outlet / pitch)))) * minus_inc_i_stall_pitch_width
...
# figure 2.29
def EW_bf(inet_angle, outlet_angle, pitch, width, velocity_inlet, velocity_outlet):
    ...
    y1 = [func(velocity_inlet ** 2 / velocity_outlet ** 2) for func in f1]
    fl = Fl(inet_angle, outlet_angle)
    return interp1d(fl_pitch_width_range, y1, fill_value='extrapolate')(fl * pitch / width).tolist()

def EW_loss(Npr, inet_angle, outlet_angle, pitch, width, length, velocity_inlet, velocity_outlet):
    ew_ar = EW_ar(width, length)
    ew_bf = EW_bf(inet_angle, outlet_angle, pitch, width, velocity_inlet, velocity_outlet)
    return Npr * ew_ar * ew_bf

def calculate_suction_length(inlet_angle, outlet_angle, width, a_outlet, pitch):
    t = a_outlet / math.cos(math.radians(outlet_angle))
    r = (width - t) / (2 * (1 - math.cos(math.radians(2 * inlet_angle)))) ** 0.5
    return 2 * math.pi * r * (inlet_angle / 360) + a_outlet * ((1 / (a_outlet / pitch) ** 2) - 1) ** 0.5
...
def calculate_secondary_losses(npr, pitch, height, inlet_angle, outlet_angle, velocity_inlet, velocity_outlet, width):
    b_sl = EW_bf(inlet_angle, outlet_angle, pitch, width, velocity_inlet, velocity_outlet)
    sl_ar = EW_ar(width, height)
    return b_sl * sl_ar * npr

def lossesCAC(temperature, pressure, velocity_inlet, velocity_outlet, height, pitch, width, chord, te_radius, a_outlet, 
              blade_inlet_angle, inlet_angle, outlet_angle, design_inc, ks, method_2):

    s_l = calculate_suction_length(inlet_angle, outlet_angle, width, a_outlet, pitch)
    npi = Npi(inlet_angle, outlet_angle, blade_inlet_angle, design_inc, pitch, s_l, a_outlet)

    npr = Npr(Re(temperature, pressure, velocity_outlet, chord), ks, s_l)

    Y_profil = abs(calculate_profile_losses(inlet_angle, outlet_angle, pitch, s_l, te_radius, npr, npi))
    Y_secondary = abs(calculate_secondary_losses(npr, pitch, height, inlet_angle, outlet_angle, velocity_inlet, velocity_outlet, s_l))
    annulus_losses = 0.0
    Y_sum = (Y_profil/100  + Y_secondary/100 + annulus_losses/100)
    Y_tl = None
    Y_te = None
    Y_cl = None
    fi = [math.sqrt(1 - (Y_profil/100  + Y_secondary/100  + annulus_losses/100))]
    return fi[0], Y_sum, Y_profil/100, Y_secondary/100, Y_tl, Y_te, Y_cl

# inlet_angle - угол на входе (alpha_0)
# outlet_angle - угол на выходе (alpha_1)
# blade_inlet_angle - лопаточный угол на входе (alpha_scapular_0)
# design_inc - угол атаки (i_stall), где i_stall = blade_inlet_angle - alpha_0
# axial_chord - осевая хорда ()
# throat - горло решетки (a)
# pitch -  шаг решетки ()
# te_radius - радиус выходной кромки 
# chord -  хорда
# height -  высота лопатки
# nu - кинематический коэффициент вязкости 
# ks коэффициент шереховатости 1e-6
# velocity_inlet -относительная скорость на входе 
# velocity_outlet - относительная скорость на выходе
#Пример: Ox = calculate_losses(90, 15, 90, 0, 60e-3, 0.75*60e-3,60e-3,1.5e-3,66e-3,70e-3,6e-5,1e-6,40, 160)

# Ox = calculate_losses(inlet_angle = 90, outlet_angle = -24, blade_inlet_angle = 90, design_inc = 0,
#                      width = 0.1274, a_outlet = 0.045, pitch = 0.1235, te_radius =  0.0011, chord = 0.18, height = 0.2236,
#                     nu = Mu(1147.79), ks = 1e-6, velocity_inlet = 368.01, velocity_outlet = 469.8)
# print(Ox)